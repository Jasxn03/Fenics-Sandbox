...
def update_obstacle_facets(mesh, cell_tags, old_facet_tags,
    ...
    facet_to_cell = mesh.topology.connectivity(fdim, cdim)

    # Copy old facet values to preserve tags we don’t want to change
    ft_values_updated = old_facet_tags.values.copy()

    # Loop over facets in old_facet_tags
    for local_idx, f in enumerate(old_facet_tags.indices):
        cells = facet_to_cell.links(f)
        if len(cells) == 0:
            continue  # isolated facet

        # Get the tags of the adjacent cells
        tags = cell_tags.values[cells]

        # Case 1: mixed fluid-biofilm → obstacle
        if fluid_tag in tags and biofilm_tag in tags:
            ft_values_updated[local_idx] = obstacle_tag

        # Case 2: biofilm-biofilm facet where one cell became fluid → obstacle
        elif all(t == biofilm_tag for t in tags) and any(cell_tags.values[c] != biofilm_tag for c in cells):
            ft_values_updated[local_idx] = obstacle_tag

        # Case 3: otherwise leave tag unchanged (fluid-fluid, inlet, wall, etc.)
        else:
            pass

    # Build new MeshTags
    new_facet_tags = meshtags(mesh, fdim, old_facet_tags.indices, ft_values_updated)
    return new_facet_tags


new_ft_updated = update_obstacle_facets(
    mesh,
    new_ct,          # updated cell tags after erosion
    new_ft,          # old facet tags before update
    fluid_tag=1,
    biofilm_tag=6,
    obstacle_tag=5
)
...
u_wall.interpolate(shear_velocity_f)
bc_shear_velocity = dirichletbc(
    u_wall, locate_dofs_topological(V, fdim, new_ft_updated.find(wall_marker))
)

# biofilm
u_nonslip = np.array((0,) * mesh.geometry.dim, dtype=PETSc.ScalarType)
bcu_obstacle = dirichletbc(
    u_nonslip, locate_dofs_topological(V, fdim, new_ft_updated.find(obstacle_marker)), V
)
             
bcu = bc_shear_velocity, bcu_obstacle

coords = mesh.geometry.x
x_target = 25 # GOT UP TO HERE NEED TO CHANGE PROPERLY, WAS STRESS WAS PLOTTED OKAYYY
y_target = 0
dist2 = (coords[:, 0] - x_target)**2 + (coords[:, 1] - y_target)**2
dof_p_ref = np.array([np.argmin(dist2)], dtype=np.int32)
bcp_ref = dirichletbc(PETSc.ScalarType(0), dof_p_ref, Q)
